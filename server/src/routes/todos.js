...
router.get('/', async (req, res) => {
  try {
    const result = await pool.query(`SELECT * FROM todos WHERE student_id=$1 ORDER BY is_completed ASC, due_date ASC NULLS LAST, created_at DESC`, [req.user.id]);
...
router.post('/', async (req, res) => {
  ...
  try {
    const result = await pool.query(`INSERT INTO todos (student_id, title, subject, due_date) VALUES ($1,$2,$3,$4) RETURNING *`, [req.user.id, title, subject, due_date || null]);
